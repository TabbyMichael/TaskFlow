...
import { sprints, tasks, burndown, velocityTrend, projects } from "@/shared/api/mock-data";
...
export function SprintsPage() {
  ...
  const activeTasks = tasks.filter((t) => t.sprintId === active.id);
  const remaining = activeTasks.filter((t) => t.status !== "done").length;
  const completed = activeTasks.length - remaining;
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="grid gap-4 sm:grid-cols-4">
                <div><p className="text-xs text-muted-foreground">Completed</p><p className="font-mono text-2xl font-semibold">{completed}</p></div>
            ...
            {sprints.map((s) => (
              <Card key={s.id}>
                <CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center">
                  ...
                  <div className="w-full sm:w-48"><div className="mb-1 flex justify-between text-xs"><span className="text-muted-foreground">Progress</span><span className="font-mono">{s.completedPoints}/{s.totalPoints}</span></div><Progress value={(s.completedPoints / s.totalPoints) * 100} className="h-1.5" /></div>
