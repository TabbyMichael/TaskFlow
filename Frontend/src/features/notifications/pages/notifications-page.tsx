...
import { notifications as initial } from "@/shared/api/mock-data";
  ...
  return (
    <div>
      <PageHeader title="Notifications" description={`${unread} unread`}
        actions={<Button variant="outline" onClick={() => setList((l) => l.map((n) => ({ ...n, read: true })))}><Check className="mr-1.5 h-4 w-4" /> Mark all read</Button>} />
            ...
            {list.length === 0 ? (
              ...
              <ul className="divide-y">
                {list.map((n) => {
                  ...
                  const actor = members?.find((u) => u.id === n.actorId);
                  return (
                    <li key={n.id} className={cn("flex items-start gap-3 p-4 hover:bg-muted/40", !n.read && "bg-primary/5")}>
                      <div className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-muted text-muted-foreground"><Icon className="h-4 w-4" /></div>
                      ...
                      {!n.read && <Button variant="ghost" size="sm" onClick={() => setList((l) => l.map((x) => x.id === n.id ? { ...x, read: true } : x))}>Mark read</Button>}
                    </li>
                  );
                })}
              </ul>
